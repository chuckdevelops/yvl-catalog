...
class AudioPlayerManager {
    ...
    setupPlayerEvents(player) {
        ...
        element.addEventListener('stalled', () => {
            console.warn(`[AudioManager] Playback stalled: ${player.element.dataset.playerId}`);
            console.log(`[AudioManager] Current source URL: ${element.src}`);
            
            if (statusElement) {
                statusElement.textContent = 'Audio playback stalled. Trying alternatives...';
                statusElement.style.display = 'block';
            }
            ...
            if (sourceElement && sourceElement.dataset.externalSource) {
                ...
                if (songIdMatch && songIdMatch[1]) {
                    ...
                    console.log(`[AudioManager] Using song ID for stalled player: ${songId}`);
                    sourceElement.src = newUrl;
                    element.load();
                    setTimeout(() => {
                        element.play().catch(err => {
            ...
            // Fall back to regular reload if no song ID was found
            this.reloadPlayer(player);
        });
        
        // Waiting for data
        element.addEventListener('waiting', () => {
            console.log(`[AudioManager] Waiting for data: ${player.element.dataset.playerId}`);
            
            if (statusElement) {
                statusElement.textContent = 'Loading audio data...';
                statusElement.style.display = 'block';
            }
        });
